...
STUDIOプラン以上のテナントオーナー向けに、ユーザーの**総合KODOMI値**（💸 JPYC軸 + ⚡ 応援熱量軸の合算貢献度）に応じてランクアップ式SBTの自動ミント・フラグNFTの自動配布を行うシステムです。

### 総合KODOMI計算式

```
総合KODOMI = JPYC軸（0-500pt）+ 応援軸（0-500pt）
           = 最大1000pt

JPYC軸: JPYC送信額を正規化（最大1500 JPYC = 500pt）
応援軸: 応援熱量スコアを正規化（最大800 = 500pt）
```
...
### 1. **総合KODOMI閾値設定**
- 各ランク（1-5）に到達するために必要な**総合KODOMI値**を設定
- localStorageとスマートコントラクトの両方に保存
- プロフィールページの総合KODOMIゲージに反映
- 全プランで総合KODOMI値を表示、STUDIOプラン以上で自動配布機能を利用可能
