...
async function main() {
  ...
  console.log("Account balance:", ethers.formatEther(balance), network.name === "polygon_mainnet" || network.name === "polygon_amoy" ? "MATIC" : "ETH");
  ...
  if (process.env.DEPLOYMENT_FEE) {
    ...
    if (network.name === "polygon_mainnet" || network.name === "polygon_amoy") {
      ...
      deploymentFee = ethers.parseEther("0.01");  // 0.01 ETH for testing
  ...
  console.log("Deployment fee:", ethers.formatEther(deploymentFee), network.name === "polygon_mainnet" || network.name === "polygon_amoy" ? "MATIC" : "ETH", "\n");
  ...
  await rankPlanRegistry.waitForDeployment();
  const rankPlanRegistryAddress = await rankPlanRegistry.getAddress();
  ...
  console.log("Deployment Fee:", ethers.formatEther(deploymentFeeFromContract), network.name === "polygon_mainnet" || network.name === "polygon_amoy" ? "MATIC" : "ETH");
